...
var log = bunyan.createLogger({
  name: 'todo_database',
  level: 'error',
  path: require('../../config').logging.directory + 'todo_database.log'
})
...
/*
 * Saves/Updates the todo analysis results for a specified version.
 *
 * @param {String} name The id of the package on CouchDB
 * @param {String} version The version currenty analyzed
 * @param {Ojbect} data The data from the todo analysis
 */
...
api.save = (name, version, data) => {
  ...
  .then(() => { return Promise.resolve('Todo database updated.') })
  .catch((err) => {
    if (err.statusCode === 404) {
